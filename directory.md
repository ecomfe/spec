...
该文档主要的设计目标是商业体系项目开发的目录结构保持一致，使容易理解并方便构建与管理。
...
在项目开发时，通常会使用如下[加载器配置](module.text#config)，将项目代号指向`src`。
...
`业务项目`的`src`目录内，绝大多数情况应当（SHOULD）根据`业务逻辑`划分目录结构。划分出的子目录（比如[例子](#bizdirexample)中的`biz1`）我们称为`业务目录`。
...
更多关于`包`的内容请参考 [包结构规范](package.text)
